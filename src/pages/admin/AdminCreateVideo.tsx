...
export default function AdminCreateVideo() {
  ...
  const { templates, profileReady, generateVideo } = useAffiliateVideos();
  ...
  useEffect(() => {
    document.title = "Create Video | Admin";
  }, []);
        ...
        <Card className="border-amber-500/30 bg-amber-500/5">
          <CardContent className="flex flex-col items-center py-12">
            ...
            <p className="text-muted-foreground text-center max-w-md">
              To generate videos, your user must have an affiliate profile (username).
            </p>
      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        <Card className="border-amber-500/30 bg-amber-500/5">
          <CardContent className="flex flex-col items-center py-12">
            ...
            <Button onClick={() => navigate("/admin/affiliate-videos/create-profile")}>Create Avatar Profile</Button>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const handleGenerate = async () => {
    ...
    if (result) {
      toast.success("Video generation started. It will appear in the admin list when ready.");
            ...
            {typeTemplates.map((template) => (
              ...
              >
                ...
                <CardContent className="space-y-3">
                  <p className="text-sm text-muted-foreground line-clamp-3">{template.script_text.substring(0, 150)}...</p>
                  <div className="flex items-center justify-between">
                    <Button
                      variant="ghost"
                      size="sm"
      ...
      {templates.length === 0 && (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center py-12">
            ...
            <p className="text-muted-foreground text-center max-w-md">Video templates are being set up. Please check back later.</p>
          </CardContent>
        </Card>
      )}
      ...
      {templates.length > 0 && (
        <div className="flex flex-col items-center gap-4 pt-4">
          <div className="p-4 rounded-lg bg-muted text-sm text-center max-w-md">
            ...
            <p className="text-muted-foreground">Video generation typically takes 5-10 minutes.</p>
          </div>

          <Button size="lg" onClick={handleGenerate} disabled={!selectedTemplate || isGenerating}>
            {isGenerating ? (
              <>
          ...
          {selectedTemplate && (
            <Badge variant="outline">Selected template</Badge>
          )}
        </div>
      )}
      ...
      <Dialog open={showScriptModal} onOpenChange={setShowScriptModal}>
        <DialogContent className="max-w-2xl max-h-[80vh]">
          ...
          <ScrollArea className="max-h-[60vh] pr-4">
            <div className="whitespace-pre-wrap text-sm">{viewingScript?.script}</div>
          </ScrollArea>
        </DialogContent>
      </Dialog>
    </div>
