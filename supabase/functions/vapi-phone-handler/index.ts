...
// EverLaunch Phone Greeter assistant ID
const ASSISTANT_ID = "40c5b4e0-0c06-4a06-a13f-76c24a397ab0";
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Vapi phone handler received:', JSON.stringify(body, null, 2));

    // Determine event type from Vapi payload
    const messageType = body.message?.type || body.type;
    console.log('Event type:', messageType);

    // Handle assistant-request: Tell Vapi which assistant to use
    if (messageType === 'assistant-request') {
      console.log('Returning assistant ID:', ASSISTANT_ID);
      return new Response(
        JSON.stringify({ assistantId: ASSISTANT_ID }),
        { 
          status: 200,
    ...
    // Handle end-of-call-report: Send transcript email
...
${transcript}
    ...
    // For all other events, return acknowledgment
    console.log('Acknowledging event:', messageType);
    return new Response(
    ...
    console.error('Error in vapi-phone-handler:', errorMessage);
    return new Response(
